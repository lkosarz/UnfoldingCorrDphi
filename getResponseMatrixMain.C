...
void getResponseMatrix(TString inputName = "data/file.list",
		TString outFileName = "outputResponse.root", long nevents = -1);
...
*/
	...
	for (int ev = 0; ev < nEvents; ++ev) {
		...
		h_dphi_read->Fill(deltaPhi);

		// select rho
		if(pt >= 0.065) continue;
		if(mass <= 0.7) continue;
		if(mass >= 0.9) continue;

		// fill true

		h_dPhiTrue->Fill(deltaPhi);

		h_dphi_rho->Fill(deltaPhi);

		h_dphi_acc->Fill(deltaPhi, accepted);
/*
		...
		for (int i = 0; i < nOrder; ++i) {
			...
			h_cosNdPhiTrue[i]->Fill(TMath::Cos(((float)i+1.0)*deltaPhi));
		...
		if(accepted == 1)
		{
			...
			response_dphi->Fill(deltaPhi, deltaPhi);

			for (int i = 0; i < nOrder; ++i) {

				h_cosNdPhiMeas[i]->Fill(TMath::Cos(((float)i+1.0)*deltaPhi));

				response_cosNdPhi[i]->Fill(TMath::Cos(((float)i+1.0)*deltaPhi), TMath::Cos(((float)i+1.0)*deltaPhi));
			...
			response_dphi->Miss(deltaPhi);

			for (int i = 0; i < nOrder; ++i) {

				response_cosNdPhi[i]->Miss(TMath::Cos(((float)i+1.0)*deltaPhi));
...
/*
	...
	TH2D *h_Resp_dPhi = (TH2D*)response_dphi->Hresponse();

	h_Resp_dPhi->GetXaxis()->SetTitle("#Delta#phi_{meas} [1]");
	h_Resp_dPhi->GetYaxis()->SetTitle("#Delta#phi_{true} [1]");
	h_Resp_dPhi->GetZaxis()->SetTitle("counts [1]");
	h_Resp_dPhi->SetName("h_Resp_dPhi");
	...
	for (int i = 0; i < nOrder; ++i) {
		...
		h_Resp_cosNdPhi[i] = (TH2D*)response_cosNdPhi[i]->Hresponse();

		h_Resp_cosNdPhi[i]->SetTitle(Form("cos(%i#Delta#phi) response matrix", i+1));
		h_Resp_cosNdPhi[i]->GetXaxis()->SetTitle(Form("cos(%i#Delta#phi)_{meas} [1]", i+1));
		h_Resp_cosNdPhi[i]->GetYaxis()->SetTitle(Form("cos(%i#Delta#phi)_{true} [1]", i+1));
		h_Resp_cosNdPhi[i]->GetZaxis()->SetTitle("counts [1]");
		h_Resp_cosNdPhi[i]->SetName(Form("h_Resp_cos%idPhi", i+1));

		response_cosNdPhi[i]->Write();
		h_Resp_cosNdPhi[i]->Write();
	}
